...
# ========== 搜索参数空间 ==========
n_embeds=(128 64 256 512)
d_models=(64)
entropy_penalties=(0.05 0.1 0.2)
entropy_temps=(0.5 1.0 1.5)
...
for entropy_temp in "${entropy_temps[@]}"; do
    ...
    # 保存路径（用于模型、日志）
    combo_name="emb${n_embed}_d${d_model}_ep${entropy_penalty}_et${entropy_temp}_wl${wave_length}_bl${block_num}_${vq_model}"
