...
# 创建统一的父目录以及子目录
mkdir -p ./exp_Wind/logs
mkdir -p ./exp_Wind/checkpoints
...
# 可变参数列表
n_embeds=(256)
entropy_penalties=(0.2 0.1 0.3 0.4 0.5)
entropy_temps=(0.1)
d_models=(64)
    ...
    for n_embed in "${n_embeds[@]}"; do
        for entropy_penalty in "${entropy_penalties[@]}"; do
            for entropy_temp in "${entropy_temps[@]}"; do
                ...
                save_path="./exp_Wind/checkpoints/${tag}"
                log_file="./exp_Wind/logs/${tag}.log"
